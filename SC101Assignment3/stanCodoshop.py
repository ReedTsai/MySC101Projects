...
def solve(images):
    ...
    for i in range(width):
        for j in range(height):  # 對每一顆pixel進行處理
            pixels = []
            for image in images:  # 取出每一張照片的pixel
                pixel = image.get_pixel(i, j)
                pixels.append(pixel)
            best1 = get_best_pixel(pixels)  # 每張照片相同位置的pixel進行比對
            result.get_pixel(i, j).red = best1.red
            result.get_pixel(i, j).green = best1.green
            result.get_pixel(i, j).blue = best1.blue
